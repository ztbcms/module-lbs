...
use app\lbs\model\LbsGeohashModel;

class LbsGeohashService extends BaseService
{
    /**
     * @var \Geohash
     */
    ...
    {
        include_once(app_path().'libs/Geohash.php');
    ...
    {
        $LbsGeohashModel = new LbsGeohashModel();
        $record = $LbsGeohashModel->where([
        ...
        if ($record) {
            $LbsGeohashModel->where(['id' => $record['id']])->update($data);
        } else {
            $LbsGeohashModel->insert($data);
        }
    ...
    function geoRemove($target_type, $target_id){
        $LbsGeohashModel = new LbsGeohashModel();
        $res = $LbsGeohashModel->where(['$target_type' => $target_type, '$target_id' => $target_id])->delete();
        if($res){
    ...
    function geoRadius($target_type, $latitude, $longitude, $radius)
    {
        ...
        $LbsGeohashModel = new LbsGeohashModel();
        if($geohash_like_list){
            $LbsGeohashModel = $LbsGeohashModel->where('geohash','like',$geohash_like_list,'OR');
        }
        $where = [
            'target_type' => $target_type,
        ];
        $result_list = $LbsGeohashModel->where($where)->select();
